...
function isLikelyEcho(transcript: string, ttsBuffer: TTSBufferEntry[]): boolean {
  ...
  for (const entry of ttsBuffer) {
    ...
    if (similarity >= ECHO_SIMILARITY_THRESHOLD) {
      console.log(
        `[Echo Detection] Blocked echo: "${transcript}" matches TTS "${entry.text}" (similarity: ${similarity.toFixed(2)})`
      );
      return true;
    }

    if (
      entry.normalizedText.includes(normalizedTranscript) ||
      ...
      if (matchLength / maxLength >= 0.5) {
        console.log(
          `[Echo Detection] Blocked partial echo: "${transcript}" contained in/contains TTS "${entry.text}"`
        );
        return true;
      }
    }
...
export const AudioControls = forwardRef<AudioControlsHandle, AudioControlsProps>(
  function AudioControls({ sessionId, sourceLanguage, targetLanguage, isActive }, ref) {
    ...
    const { status, volume, isMuted, setStatus, setTargetLanguage, setVolume, toggleMute } =
      useTranslationStore();
    ...
    const sendTextForTranslationRef = useRef<((text: string, segmentId: string) => void) | null>(
      null
    );
    ...
    const sendTextForTranslation = useCallback(
    ...
    const sendSegmentToBackend = useCallback(
      (segment: {
        id: string;
        text: string;
        start_time: number;
          ...
          >
            ...
            >
              ...
              <Typography
                variant="caption"
                color={
                  isPaused ? '#F59E0B' : isTranscribing ? customColors.brandGreen : 'text.disabled'
                }
              >
              ...
              sx={{
                ...
                bgcolor: isMuted
                  ...
                  : translationConnected
                    ...
                    : 'transparent',
                '&:hover':
                  translationConnected || isMuted
                    ? {
                        bgcolor: isMuted ? alpha('#999', 0.1) : alpha(customColors.brandGreen, 0.2),
                      }
                    : {},
              }}
            >
